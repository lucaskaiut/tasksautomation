...
<div class="space-y-6">
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div>
            ...
            <select id="environment_profile_id" name="environment_profile_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                <option value="">Sem</option>
                @foreach ($environmentProfiles as $profile)
                    <option value="{{ $profile->id }}" @selected((string) $selectedEnvironmentProfileId === (string) $profile->id)>
                        {{ $profile->project?->name }} — {{ $profile->name }} ({{ $profile->slug }})
                    </option>
                @endforeach
            </select>
